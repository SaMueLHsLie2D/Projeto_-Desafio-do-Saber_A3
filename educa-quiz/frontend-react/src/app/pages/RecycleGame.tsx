...
function DraggableItem({ item, onDrop }: DraggableItemProps) {
  ...
  return (
    <motion.div
      ...
      } item-card bg-white rounded-2xl p-4 shadow-lg cursor-move hover:shadow-xl transition-all`}
    >
...
function RecycleBin({ type, color, label, emoji, onDrop }: BinProps) {
  ...
  return (
    <div
      ref={drop}
      className={`${color} recycle-bin-card rounded-3xl p-6 shadow-xl transition-all ${
...
export default function RecycleGame() {
  ...
  const handleDrop = (droppedItemId: string, binType: string) => {
    ...
    // Only remove item when the drop is correct
    if (isCorrect) {
  ...
  const handleItemDrop = (itemId: string) => {
    // Intentionally left: main removal is handled by bins' drop. Keep for future use.
  };
